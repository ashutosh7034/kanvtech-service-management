...
export class ImportService {
  /**
   * Generates downloadable Excel template buffer for Companies or Employees.
   */
  public static generateTemplate(type: 'companies' | 'employees'): Buffer {
    let headers: string[] = [];
    let sampleData: any[] = [];
  ...
  /**
   * Parses and validates company import rows.
   */
  public static async validateCompanyImport(fileBuffer: Buffer): Promise<ImportPreviewResult> {
    ...
    // Fetch existing emails & GSTNs from DB
    const existingCompanies = await db.query<{ primary_email: string; gstn: string }>(
      'SELECT LOWER(primary_email) as primary_email, gstn FROM companies'
    );
    const dbEmails = new Set(existingCompanies.map((c) => c.primary_email));
  ...
  /**
   * Commits validated company rows to database.
   */
  public static async commitCompanyImport(rows: any[], actorUserId?: number): Promise<{ importedCount: number }> {
    let importedCount = 0;
    for (const row of rows) {
      await CompanyService.createCompany(row, actorUserId);
      importedCount++;
    }
  ...
  /**
   * Parses and validates employee import rows.
   */
  public static async validateEmployeeImport(fileBuffer: Buffer): Promise<ImportPreviewResult> {
    const wb = XLSX.read(fileBuffer, { type: 'buffer' });
    const sheetName = wb.SheetNames[0];
    const rawRows: any[] = XLSX.utils.sheet_to_json(wb.Sheets[sheetName]);
    ...
    const existingUsers = await db.query<{ email: string }>('SELECT LOWER(email) as email FROM users');
  ...
  /**
   * Commits validated employee rows to database.
   */
  public static async commitEmployeeImport(rows: any[], actorUserId?: number): Promise<{ importedCount: number }> {
    let importedCount = 0;
    for (const row of rows) {
      await EmployeeService.createEmployee(row, actorUserId);
      importedCount++;
    }
    return { importedCount };
  }
