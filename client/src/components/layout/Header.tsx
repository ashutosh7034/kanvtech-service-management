...
import { Bell, User, CheckCircle2, ChevronDown, Check } from 'lucide-react';
...
export const Header: React.FC<Props> = ({ pageTitle }) => {
  ...
  const handleRoleSwitch = async (role: UserRole) => {
    ...
    showToast(`Switched view to ${role.replace('_', ' ')} role`, 'info');
  };
      ...
      <div className="header-right">
        ...
        <div style={{ position: 'relative' }} ref={roleMenuRef}>
          ...
          >
            ...
            <strong style={{ color: '#0f172a' }}>{user?.role.replace('_', ' ')}</strong>
          ...
          {showRoleMenu && (
            ...
            >
              <div style={{ padding: '6px 12px', fontSize: 11, fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase' }}>
                Simulate Perspective
              </div>
              {Object.entries(DEMO_CREDENTIALS).map(([rKey, cred]) => (
                <button
                  key={rKey}
                  onClick={() => handleRoleSwitch(rKey as UserRole)}
                  style={{
                    width: '100%',
                    padding: '8px 14px',
                    textAlign: 'left',
                    background: user?.role === rKey ? '#f0f9ff' : 'transparent',
                    border: 'none',
                    cursor: 'pointer',
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <div>
                    <div style={{ fontWeight: 600, color: '#0f172a' }}>{cred.label}</div>
                    <div style={{ fontSize: 11, color: '#64748b' }}>{cred.desc}</div>
                  </div>
                  {user?.role === rKey && <Check size={14} color="#0284c7" />}
                </button>
              ))}
            </div>
          )}
